...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="bg-slate-900 text-slate-200 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <i className="fa-solid fa-circle-notch animate-spin text-3xl text-orange-500 mb-4"></i>
  ...
  const menuItems = [
    ...
    { name: "Ambassadors", path: "/admin/ambassadors", icon: "fa-users" },
    { name: "Referrals", path: "/admin/referrals", icon: "fa-user-plus" },
    { name: "Reviews", path: "/admin/reviews", icon: "fa-star" },
    { name: "Payout Requests", path: "/admin/payouts", icon: "fa-money-bill-transfer" },
    { name: "Tier Settings", path: "/admin/tier-settings", icon: "fa-layer-group" },
  ];
    ...
    <div className={`${theme} bg-slate-900 text-slate-200 flex h-screen overflow-hidden`}>
      ...
      <aside className="hidden lg:flex w-64 bg-slate-950/80 border-r border-white/10 p-6 flex-shrink-0 flex flex-col">
        <div className="flex flex-col gap-1 mb-8">
          ...
          <span className="text-orange-400 font-extrabold text-[10px] tracking-widest uppercase text-center mt-1">
            Admin Panel
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="h-16 border-b border-white/10 px-4 sm:px-8 flex items-center justify-between bg-slate-950/40">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={toggleTheme}
              className="text-slate-450 hover:text-orange-500 hover:bg-slate-800/20 p-2 rounded-lg transition-all flex items-center justify-center cursor-pointer"
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-orange-500/20 text-orange-400 flex items-center justify-center font-bold text-xs">
                {user.full_name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2) || "AD"}
              </div>
            ...
            <button
              onClick={logout}
              className="text-slate-400 hover:text-orange-400 text-xs font-bold flex items-center gap-1.5 transition-colors"
            >
